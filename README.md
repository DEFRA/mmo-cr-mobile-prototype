...
> [!WARNING]
> The `mmo-cr-mobile-prototype` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `mmo-cr-mobile-prototype`. To turn it
...
```bash
docker build --target development --no-cache --tag mmo-cr-mobile-prototype:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 mmo-cr-mobile-prototype:development
```
...
```bash
docker build --no-cache --tag mmo-cr-mobile-prototype .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 mmo-cr-mobile-prototype
```
...
```bash
docker run -it --entrypoint /bin/ash mmo-cr-mobile-prototype
```
